...
class User(Base, DateTimeMixin):
    ...
    email = Column(
        String, unique=True, index=True, nullable=False, comment="Почта пользователя"
    )
    hashed_password = Column(
        String, nullable=False, comment="Хешированный пароль пользователя"
    )
    fio = Column(String, nullable=True, comment="ФИО пользователя")
    is_active = Column(
        Boolean, default=True, nullable=False, comment="Активность пользователя"
    )
    is_superuser = Column(
        Boolean, default=False, nullable=False, comment="Полный доступ пользователя"
    )
    role = Column(
        String,
        nullable=False,
