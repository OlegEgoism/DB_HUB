...
class DB_Connection(Base, DateTimeMixin):
    ...
    name = Column(
        String(255), nullable=False, index=True, comment="Название подключения"
    )
...
class DB_User(Base, DateTimeMixin):
    ...
    name = Column(
        String(255), nullable=False, index=True, comment="Название группы/пользователя"
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "connection_id", "oid", "name", name="uq_db_user_connection_name"
        ),
