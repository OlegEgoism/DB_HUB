...
async def get_schema_statistics(
    ...
    try:
        ...
        result = await service.get_schemas_with_statistics(connection_id=connection_id, search=search, page=page, size=size, sort_by=sort_by, sort_order=sort_order)
        return result
...
async def create_schema(connection_id: int, create_data: DBSchemaCreateRequest, db: AsyncSession = Depends(get_db)):
    """Создать новую схему во внешней БД."""
    try:
        service = DBSchemaService(db)
        result = await service.create_schema(connection_id=connection_id, name=create_data.name, owner=create_data.owner, description=create_data.description)
...
async def delete_schema(connection_id: int, delete_data: DBSchemaDeleteRequest, db: AsyncSession = Depends(get_db)):
    """Удалить схему из внешней БД."""
    try:
        service = DBSchemaService(db)
        result = await service.delete_schema(connection_id=connection_id, schema_name=delete_data.schema_name, cascade=delete_data.cascade)
...
async def update_schema(connection_id: int, schema_oid: int, update_data: DBSchemaUpdateRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        result = await service.update_schema(connection_id=connection_id, schema_oid=schema_oid, name=update_data.name, description=update_data.description)
