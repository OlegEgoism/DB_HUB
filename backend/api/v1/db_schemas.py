...
router = APIRouter(
    prefix="/db_connections/{connection_id}/schemas", tags=["DB SCHEMAS"]
)
...
async def get_schema_privileges_for_users(
    ...
    search: str = Query(
        None, description="Поиск по схеме названию/описанию и имени пользователя"
    ),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при получении привилегий схем: {str(e)}"
        )
...
async def update_schema_privileges_for_users(
    ...
    try:
        ...
        updated = await service.update_schema_privileges_for_users(
            ...
            user_privileges=[
                {"username": u.username, "create": u.create, "usage": u.usage}
                for u in request.users
            ],
        )
        return SchemaPrivilegesUpdateResponse(
            message="Права успешно обновлены", updated_users=updated
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Ошибка при обновлении прав: {str(e)}"
        )


@router.get(
    "/privileges_groups", response_model=PaginatedSchemaPrivilegesGroupsResponse
)
async def get_schema_privileges_for_groups(
    ...
    search: str = Query(
        None, description="Поиск по схеме названию/описанию и имени группы"
    ),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при получении привилегий групп: {str(e)}"
        )
...
async def update_schema_privileges_for_groups(
    ...
    try:
        ...
        updated = await service.update_schema_privileges_for_groups(
            ...
            schema_name=request.schema_name,
            group_privileges=[
                {"groupname": g.groupname, "create": g.create, "usage": g.usage}
                for g in request.groups
            ],
        )
        return SchemaPrivilegesGroupsUpdateResponse(
            message="Права для групп успешно обновлены", updated_groups=updated
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Ошибка при обновлении прав для групп: {str(e)}"
        )
