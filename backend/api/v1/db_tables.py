...
async def get_tables(
    ...
    search: str = Query(
        None, description="Поиск по имени/описанию схемы, имени/описанию таблицы"
    ),
):
...
async def get_tables_temporary(
    ...
    try:
        service = DBTablesService(db)
        result = await service.get_tables_temporary(
            connection_id=connection_id, page=page, size=size, search=search
        )
        return result
    except ValueError as e:
...
async def get_tables_privileges_for_users(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при получении привилегий таблиц: {str(e)}"
        )
...
async def update_tables_privileges_for_users(
    ...
    try:
        ...
        return TablePrivilegesUpdateResponse(
            message="Права на таблицу успешно обновлены", updated_users=updated
        )
    except ValueError as e:
...
async def update_tables_privileges_for_groups(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при обновлении прав групп: {str(e)}"
        )
