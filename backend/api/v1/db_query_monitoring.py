...
router = APIRouter(
    prefix="/db_connections/{connection_id}", tags=["DB QUERY MONITORING"]
)
...
async def get_slow_queries(
    ...
    min_duration_ms: int = Query(
        1000, ge=0, description="Минимальная длительность запроса в миллисекундах"
    ),
    ...
    search: str = Query(
        None, description="Поиск по тексту запроса, имени пользователя или приложению"
    ),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при получении медленных запросов: {str(e)}"
        )
