...
async def list_users(
    ...
    search: Optional[str] = Query(
        None, description="Поиск по имени/описанию пользователя"
    ),
):
...
async def get_user(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при получении пользователя: {str(e)}"
        )
...
async def update_user(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при обновлении пользователя: {str(e)}"
        )
...
async def delete_user(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при удалении пользователя: {str(e)}"
        )
...
async def get_user_with_groups(
    ...
    try:
        service = DBUserService(db)
        return await service.get_user_with_groups(
            connection_id=connection_id, user_oid=oid
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
