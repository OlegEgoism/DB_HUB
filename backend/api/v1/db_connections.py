...
async def list_connections(
    ...
    search: Optional[str] = Query(
        None, description="Поиск по названию/описанию базы данных"
    ),
    ...
    try:
        ...
        if search and search.strip():
            ...
            filters.append(
                or_(
                    DB_Connection.database_name.ilike(term),
                    DB_Connection.name.ilike(term),
                    DB_Connection.description.ilike(term),
                )
            )
        if database_type:
        ...
        total = (
            await db.execute(query.with_only_columns(func.count(DB_Connection.id)))
        ).scalar_one()
        query = (
            query.order_by(DB_Connection.name.asc())
            .offset((page - 1) * size)
            .limit(size)
        )
        ...
        for row in rows:
            ...
            status_conn, size_mb, db_description, is_connected = await get_db_info(
                connection
            )
            effective_description = connection.description
            if is_connected:
                effective_description = await sync_description_if_needed(
                    db, connection, db_description
                )
            items.append(
                build_connection_out(
                    connection,
                    owner_username,
                    status_conn,
                    size_mb,
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при получении списка подключений: {str(e)}"
        )
...
async def get_connection(connection_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return build_connection_out(
        connection, owner_username, status_conn, size_mb, effective_description
    )


@router.post("", response_model=ConnectionOut, status_code=201)
async def create_connection(
    connection: ConnectionCreate, db: AsyncSession = Depends(get_db)
):
    """Создать подключение"""
    if not (
        await db.execute(select(User).where(User.id == connection.owner_id))
    ).scalar_one_or_none():
        raise HTTPException(status_code=400, detail="Владелец не найден")
    encrypted_password = encrypt_password(connection.password)
    db_connection = DB_Connection(
        **connection.model_dump(exclude={"password"}), password=encrypted_password
    )
    db.add(db_connection)
    await db.commit()
    await db.refresh(db_connection)
    owner_username = (
        await db.execute(select(User.username).where(User.id == db_connection.owner_id))
    ).scalar_one()
    ...
    if is_connected:
        effective_description = await sync_description_if_needed(
            db, db_connection, db_description
        )
    return build_connection_out(
        db_connection, owner_username, status_conn, size, effective_description
    )
...
async def update_connection(
    ...
    if is_connected:
        ...
        if "description" in update_data:
            try:
                async with external_db_connection(
                    ...
                    await ext_conn.execute(
                        f"COMMENT ON DATABASE {quoted_db} IS $${comment_value}$$"
                    )
            except Exception:
                pass
    owner_username = (
    ...
    return build_connection_out(
        db_connection,
        owner_username,
        "connected" if is_connected else "error",
        None,
        effective_description,
    )
...
@router.get(
    "/{connection_id}/active_connections",
    response_model=PaginatedActiveConnectionsResponse,
)
...
async def delete_active_connection(
    connection_id: int,
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при завершении процесса: {str(e)}"
        )


@router.patch("/{connection_id}/favorite", response_model=ConnectionOut)
async def update_connection_favorite(
    connection_id: int,
    favorite_update: ConnectionFavoriteUpdate,
    db: AsyncSession = Depends(get_db),
):
