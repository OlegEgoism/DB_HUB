...
class DBGroupService:
    ...
    async def get_group(self, connection_id: int, group_oid: int) -> DBGroupOut:
        ...
        async with external_db_connection(connection) as conn:
            ...
            if not row:
                raise ValueError(
                    f"Группа с oid {group_oid} не найдена или является системной"
                )
            return DBGroupOut(
                oid=row["oid"],
                name=row["name"],
    ...
    async def create_group(
        self, connection_id: int, create_data: "DBGroupCreate"
    ) -> dict:
        connection = await self.get_connection(connection_id)
        if not connection:
            raise ValueError("Подключение не найдено")
        ...
        async with external_db_connection(connection) as conn:
            exists = await conn.fetchval(
                "SELECT 1 FROM pg_roles WHERE rolname = $1", group_name
            )
            if exists:
            ...
            oid = await conn.fetchval(
                "SELECT oid FROM pg_roles WHERE rolname = $1", group_name
            )
            if not oid:
    ...
    async def update_group(
        self, connection_id: int, group_oid: int, update_data: "DBGroupUpdate"
    ) -> dict:
        connection = await self.get_connection(connection_id)
        if not connection:
            raise ValueError("Подключение не найдено")
        ...
        async with external_db_connection(connection) as conn:
            ...
            if new_name is not None and new_name != current_name:
                ...
                taken = await conn.fetchval(
                    "SELECT 1 FROM pg_roles WHERE rolname = $1", new_name
                )
                if taken:
    ...
    async def add_user_to_group(
        self, connection_id: int, user_oid: int, group_oid: int
    ) -> dict:
        ...
        async with external_db_connection(connection) as conn:
            ...
            if not user_row:
                raise ValueError(
                    f"Пользователь с oid {user_oid} не найден или является системным"
                )
            group_row = await conn.fetchrow(
                """
                SELECT rolname FROM pg_roles
