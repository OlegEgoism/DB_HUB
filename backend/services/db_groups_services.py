...
class DBGroupService:
        ...
        try:
                ...
                if local.oid not in external_oids:
                    if local.name in existing_by_name and local.name in [g["name"] for g in external_groups]:
                        ...
                        if external_with_same_name["oid"] != local.oid:
                            ...
                            deleted_groups.append({"oid": local.oid, "name": local.name, "description": local.description, "reason": "oid_changed_same_name"})
                            stats["deleted"] += 1
                            continue
                    await self.db.delete(local)
                    deleted_groups.append({"oid": local.oid, "name": local.name, "description": local.description, "reason": "deleted_from_external"})
                    stats["deleted"] += 1
            result = await self.db.execute(select(DB_Group).where(DB_Group.connection_id == connection_id))
            ...
            for ext in external_groups:
                ...
                if ext_oid in current_by_oid:
                    ...
                    deleted_groups.append({"oid": old_local.oid, "name": old_local.name, "description": old_local.description, "reason": "replaced_due_to_oid_change"})
                    stats["deleted"] += 1
                    new_group = DB_Group(oid=ext_oid, name=ext_name, description=ext["external_description"], connection_id=connection_id)
                    self.db.add(new_group)
                    added.append({"oid": ext_oid, "name": ext_name, "description": ext["external_description"], "reason": "name_preserved_new_oid"})
                    stats["added"] += 1
                else:
                    new_group = DB_Group(oid=ext_oid, name=ext_name, description=ext["external_description"], connection_id=connection_id)
                    self.db.add(new_group)
                    added.append({"oid": ext_oid, "name": ext_name, "description": ext["external_description"], "reason": "new_group"})
                    stats["added"] += 1
    ...
    async def create_group(self, connection_id: int, name: str, description: Optional[str] = None) -> Dict[str, Any]:
        ...
        db_group = DB_Group(oid=oid, name=name, description=description, connection_id=connection_id)
    ...
    async def delete_group(self, group_id: int) -> Dict[str, Any]:
        ...
        await self.db.commit()
        return {"message": f"Группа '{group_name}' успешно удалена из внешней и локальной баз данных", "deleted_group_id": group_id, "group_name": group_name, "connection_id": connection_id, }
    ...
    async def get_group_members_from_external_db(self, group_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            await conn.close()
            members = [
                {"user_oid": row["user_oid"], "username": row["username"], "rolsuper": row["rolsuper"]}
                for row in rows
            ]
            return {
                "group_id": group.id,
