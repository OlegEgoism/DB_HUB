...
class DBTablesService:
    ...
    async def get_tables(self, connection_id: int, page: int = 1, size: int = 20, search: Optional[str] = None) -> Dict[str, Any]:
                ...
                for s in all_schemas:
                    ...
                    for t in table_rows:
                        ...
                        if search_term in tbl_name or search_term in tbl_desc:
                            matching_tables.append({"table_name": t["table_name"], "table_description": t["table_description"]})
                    if schema_matches or matching_tables:
                        schemas_with_matching_tables.append({"schema": s, "matching_tables": matching_tables if not schema_matches else []})
            filtered_schemas = []
        ...
        async with external_db_connection(connection) as conn:
            ...
            table_info = {
                row["table_oid"]: {
                    ...
                    "owner": row["owner"],
                    "privileges": {
                        username: {"SELECT": False, "INSERT": False, "UPDATE": False, "DELETE": False, "TRUNCATE": False}
        ...
        async with external_db_connection(connection) as conn:
            ...
            table_info = {
                row["table_oid"]: {
                    ...
                    "owner": row["owner"],
                    "privileges": {
                        groupname: {"SELECT": False, "INSERT": False, "UPDATE": False, "DELETE": False, "TRUNCATE": False}
