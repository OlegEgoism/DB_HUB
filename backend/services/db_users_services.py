...
class DBUserService:
    ...
    async def sync_users_from_external_db(self, connection_id: int) -> None:
        ...
        result = await self.db.execute(
            select(DB_User).where(DB_User.connection_id == connection_id)
        )
        ...
        for oid, ext_user in external_users.items():
            ...
            if local_user:
                changed = (
                    ...
                    or local_user.description != ext_user["description"]
                )
                if changed:
                    local_user.name = ext_user["name"]
                ...
                new_user = DB_User(
                    ...
                    oid=ext_user["oid"],
                    name=ext_user["name"],
                    description=ext_user["description"],
                )
                self.db.add(new_user)
        if local_users:
            oids_to_delete = list(local_users.keys())
            await self.db.execute(
                delete(DB_User).where(
                    DB_User.connection_id == connection_id,
                    DB_User.oid.in_(oids_to_delete),
                )
            )
    ...
    async def list_users(
        ...
        offset = (page - 1) * size
        result = await self.db.execute(
            query.order_by(DB_User.name).offset(offset).limit(size)
        )
    ...
    async def get_user(self, connection_id: int, user_oid: int) -> DBUserOut:
        await self.sync_users_from_external_db(connection_id)
        result = await self.db.execute(
            select(DB_User).where(
                DB_User.connection_id == connection_id, DB_User.oid == user_oid
            )
        )
        ...
        return DBUserOut(
            oid=user.oid, name=user.name, description=user.description, email=user.email
        )
    ...
    async def create_user(
        ...
        async with external_db_connection(connection) as conn:
            ...
            if exists_row:
                raise ValueError(
                    f"Пользователь с именем '{username}' уже существует во внешней базе"
                )
            quoted_password = await conn.fetchval("SELECT quote_literal($1)", password)
            quoted_username = _quote_ident(username)
            create_sql = (
                f"CREATE ROLE {quoted_username} WITH LOGIN PASSWORD {quoted_password}"
            )
            await conn.execute(create_sql)
            if description is not None:
                quoted_desc = await conn.fetchval(
                    "SELECT quote_literal($1)", description
                )
        ...
        result = await self.db.execute(
            select(DB_User).where(
                DB_User.connection_id == connection_id, DB_User.name == username
            )
        )
        user = result.scalar_one_or_none()
        if not user:
            raise RuntimeError(
                "Не удалось найти только что созданного пользователя после синхронизации"
            )
        if email != user.email:
            user.email = email
            self.db.add(user)
    ...
    async def delete_user(self, connection_id: int, user_oid: int) -> None:
        ...
        if not local_user:
            raise ValueError(
                f"Пользователь с oid {user_oid} не найден в локальной базе"
            )
        username = local_user.name
        ...
        async with external_db_connection(connection) as conn:
            ...
            if exists:
                try:
                    ...
                    if (
                        "required by other objects" in error_msg
                        or "dependent objects" in error_msg
                    ):
                        raise ValueError(
    ...
    async def get_user_with_groups(
        self, connection_id: int, user_oid: int
    ) -> list[DBUserOut]:
        connection = await self.get_connection(connection_id)
        if not connection:
            raise ValueError("Подключение не найдено")
