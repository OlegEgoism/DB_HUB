    ...
    async def execute_query(self, connection_id: int, query: str, limit: int = 100) -> dict[str, Any]:
        ...
        async with external_db_connection(connection, timeout=30) as conn:
            try:
                ...
                raise ValueError(f"Ошибка синтаксиса SQL: {e}") from e
            except asyncpg.exceptions.InsufficientPrivilegeError as e:
                raise ValueError(f"Недостаточно прав для выполнения запроса: {e}") from e
            except Exception as e:
                raise ValueError(f"Ошибка выполнения запроса: {e}") from e
