...
// Состояния для пагинации пользователей
    ...
    const [activeSqlWarningMessage, setActiveSqlWarningMessage] = useState<string | null>(null);
...
// Обработчики для поиска и пагинации пользователей
    ...
    const parseApiErrorDetail = (detail: unknown, fallbackMessage = 'Не удалось выполнить SQL-запрос'): string => {
    ...
    const closeActiveSqlWarningModal = () => {
        setActiveSqlWarningMessage(null);
    };
    ...
    const terminateActiveSqlQuery = async (pid: number) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    parseApiErrorDetail(
                        (errData as { detail?: unknown }).detail ?? errData,
                        'Не удалось завершить активный SQL-запрос',
                    ),
                );
            }
            ...
            if (errorMessage.includes('Процесс с PID не найден или уже завершён')) {
                setActiveSqlWarningMessage(errorMessage);
            } else {
...
<span className={clsx(styles.paginationHighlight)}>{Math.min(proceduresPage * proceduresPageSize, totalProcedures)}</span> из <span className={clsx(styles.paginationHighlight)}>{totalProcedures}</span> процедур
            ...
            {activeSqlWarningMessage !== null && (
                <div className={clsx(styles.modalOverlay)} onClick={closeActiveSqlWarningModal}>
                    <div className={clsx(styles.modalContent)} onClick={(e) => e.stopPropagation()}>
                        <div className={clsx(styles.modalHeader)}>
                            ...
                            <h2 className={clsx(styles.modalTitle)}>Предупреждение</h2>
                        </div>
                        <div className={clsx(styles.modalBody)}>
                            <p className={clsx(styles.modalText)}>{activeSqlWarningMessage}</p>
                        </div>
                        <div className={clsx(styles.modalFooter)}>
                            <button
                                className={clsx(styles.modalCancelButton)}
                                onClick={closeActiveSqlWarningModal}
                            >
                                Понятно
                            </button>
                        </div>
