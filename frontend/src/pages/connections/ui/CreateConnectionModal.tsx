// frontend/src/pages/connections/ui/CreateConnectionModal.tsx
import { useState } from 'react';
import { useCreateConnection } from '../lib/useCreateConnection';
import { useLogin } from '@pages/auth/lib/useLogin'; // Добавлен импорт
...
export function CreateConnectionModal({
  ...
  const { getUser } = useLogin(); // Получаем данные пользователя
  const currentUser = getUser(); // Текущий авторизованный пользователь
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      alert('Пожалуйста, введите название подключения');
      return;
    }
    if (!formData.host.trim()) {
      alert('Пожалуйста, введите хост');
      return;
    }
    if (!formData.database_name.trim()) {
      alert('Пожалуйста, введите имя базы данных');
      return;
    }
    ...
    try {
      const createData = {
        ...
        owner_id: currentUser.id, // Передаём реальный ID пользователя
      };
      ...
      <div className={clsx(styles.modal__content)}>
        ...
        <div className={clsx(styles.modal__header)}>
          <h2 className={clsx(styles.modal__title)}>
            {success ? 'Подключение создано!' : 'Создание подключения'}
          </h2>
        </div>
        {success ? (
          <div className={clsx(styles.modal__success)}>
            ...
            <div className={clsx(styles.modal__successHint)}>
              Ваше новое подключение добавлено в список
            </div>
